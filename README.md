...
최종 MAE similarity는 무보정 체인 `96.4261%`, 적용 범위 게이트를 통과한 매회 보정 체인 `98.2248%`로 `+1.7986%p` 높았습니다. 각 회차에서 보정 직전과 직후만 비교하면 5회 모두 양수였고 평균은 `+0.1244%p`였습니다. 3회차와 5회차에서는 큰 재생성 출력이 각각 1장씩 게이트에서 제외됐습니다. 두 체인의 단계별 차이에는 Gemini 샘플링과 입력 분기 효과도 포함되므로 보정 모델 하나의 인과 효과로만 해석하면 안 됩니다.
